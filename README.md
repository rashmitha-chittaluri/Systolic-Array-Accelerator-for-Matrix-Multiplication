# Introduction
This project implements a custom 8×8 TPU-style systolic array accelerator for matrix multiplication. The design multiplies an 8×8 weight matrix (Matrix A) with an 8×8 input/data matrix (Matrix B) using a parallel systolic architecture. Before computation, both matrices are reordered into a systolic-friendly streaming format and pushed into input queues, which feed one value per cycle into the array. Inside the grid, weights move top-to-bottom while data move left-to-right, creating a wavefront of computation where all 64 PEs work together. Each PE performs a multiply–accumulate (MAC) as values pass through it. In our implementation, the computation uses a single-stage pipelined MAC (not fully pipelined), keeping the design simpler while still enabling parallel operation. The simulation shows a throughput of 8.951 GOPS, confirming that even with a single pipeline stage, the systolic array achieves efficient matrix multiplication.
